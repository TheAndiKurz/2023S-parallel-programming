...
/**
 * This is used to verify the input file was read correctly
 */
#define CHECK_EQUAL_ONE(result, message) \
    do { \
        if((result) != 1) { \
            printf("Error: %s\n", (message)); \
            fclose(fp); \
...
int main() {
    ...
    /**
     * NOTE: Modified to add error handling
     */
    if((fp = fopen("mg.input", "r")) != NULL) {
        int result;
        printf(" Reading from input file mg.input\n");
        result = fscanf(fp, "%d\n", &lt);
        CHECK_EQUAL_ONE(result, "Failed to read 'lt' from file.");
        while(fgetc(fp) != '\n')
            ;
        result = fscanf(fp, "%d%d%d", &nx[lt], &ny[lt], &nz[lt]);
        CHECK_EQUAL_ONE(result, "Failed to read 'nx', 'ny', 'nz' from file.");
        while(fgetc(fp) != '\n')
            ;
        result = fscanf(fp, "%d", &nit);
        CHECK_EQUAL_ONE(result, "Failed to read 'nit' from file.");
        while(fgetc(fp) != '\n')
            ;
        for(i = 0; i <= 7; i++) {
            result = fscanf(fp, "%d", &debug_vec[i]);
            CHECK_EQUAL_ONE(result, "Failed to read 'debug_vec' from file.");
        }
        fclose(fp);
    } else {
